class SpltrainsController < ApplicationController
  before_action :set_spltrain, only: [:show, :edit, :update, :destroy]

  # GET /spltrains
  # GET /spltrains.json
  def index
    @spltrains = Spltrain.all
  end

  # GET /spltrains/1
  # GET /spltrains/1.json
  def show
  end

  # GET /spltrains/new
  def new
    @spltrain = Spltrain.new
  end

  # GET /spltrains/1/edit
  def edit
  end

  # POST /spltrains
  # POST /spltrains.json
  def create
    @spltrain = Spltrain.new(spltrain_params)

    respond_to do |format|
      if @spltrain.save
        format.html { redirect_to @spltrain, notice: 'Spltrain was successfully created.' }
        format.json { render :show, status: :created, location: @spltrain }
      else
        format.html { render :new }
        format.json { render json: @spltrain.errors, status: :unprocessable_entity }
      end
    end
  end

  # PATCH/PUT /spltrains/1
  # PATCH/PUT /spltrains/1.json
  def update
    respond_to do |format|
      if @spltrain.update(spltrain_params)
        format.html { redirect_to @spltrain, notice: 'Spltrain was successfully updated.' }
        format.json { render :show, status: :ok, location: @spltrain }
      else
        format.html { render :edit }
        format.json { render json: @spltrain.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /spltrains/1
  # DELETE /spltrains/1.json
  def destroy
    @spltrain.destroy
    respond_to do |format|
      format.html { redirect_to spltrains_url, notice: 'Spltrain was successfully destroyed.' }
      format.json { head :no_content }
    end
  end

  private
    # Use callbacks to share common setup or constraints between actions.
    def set_spltrain
      @spltrain = Spltrain.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def spltrain_params
      params.require(:spltrain).permit(:status, :date)
    end
end
